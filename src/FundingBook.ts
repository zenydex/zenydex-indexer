...
ponder.on("FundingBook:OfferCreated", async ({ event, context }) => {
  ...
  await context.db.update(ProtocolMetrics, { id: metricsId(chainId) }).set((prev) => ({
    ...
    totalDepositVolume: (prev.totalDepositVolume ?? 0n) + offer.amount,
...
ponder.on("FundingBook:FundingFilled", async ({ event, context }) => {
  ...
  if (offer) {
    // Don't touch cancelled offers (lender zeroed)
    if (offer.status === "CANCELED" || offer.lender === "0x0000000000000000000000000000000000000000") {
      // Skip — offer was cancelled
    } else {
